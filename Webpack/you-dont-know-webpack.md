...
2. 前者为引用输出，后者为对象的浅拷贝；因此对于`commonjs`，假若导出中有值类型，那么它只是一个拷贝。。
...
提示：es为模块引用，version的输出分别为： `1,1`, `2,2`
...
与原始es的输出一致。这里和第二问的最主要区别在于，编译后会按照es的规则转化为cjs，具体来说，对于version的处理就会绑定exports，如下：
